...
void Person::write_events(){
    ...
    for(int i = 0; i < size; ++i){
        ...
        if(e[i]->getEventType() == "ClassWork"){
            f.write(to_string(e[i]->getDueYear()).c_str(), 5);
            f.write(to_string(e[i]->getDueMonth()).c_str(), 5);
            f.write(to_string(e[i]->getDueDay()).c_str(), 5);
            f.write(to_string(e[i]->getTimeSpend()).c_str(), 5);
            f.write(to_string(e[i]->getExam()).c_str(), 5);
            f.write(e[i]->getClassName().c_str(), 25);
        }
        else{
            string tildes = "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~";
...
void Person::read_events(){
    ...
    for(int i = 0; i < 10000; i += 200){
        ...
        if (events[i + 150] != '~') {       // Is this a classwork?
            ...
            string ts = (events + i + 165);
            stringstream tss(ts);
            int timeSpend; tss >> timeSpend;

            string e = (events + i + 170);
            stringstream es(e);
            bool exam; es >> exam;

            string className = (events + i + 175);

            // Create a new ClassWork object and add it to person
            temp = new ClassWork(startYear, startMonth, startDay, startTime, endTime, priority, eventType, title, description, repeats, maxRepeats, dueYear, dueMonth, dueDay, timeSpend, exam, className);
            addEvent(temp);
        }

        else{
            // If just a regular event, create Event object and add it to person
            temp = new Event(startYear, startMonth, startDay, startTime, endTime, priority, eventType, title, description, repeats, maxRepeats);
            addEvent(temp);
        }
    }
